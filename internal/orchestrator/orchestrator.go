...
func New(opts Options) (*Orchestrator, error) {
	...
	comp, err := compiler.New(cwd)
	if err != nil {
		return nil, err
	}

	rewriter := paths.New(cwd, tsCfg.Paths, tsCfg.OutDir, tsCfg.RootDir)
...
func (o *Orchestrator) Build(ctx context.Context, isRebuild bool) error {
	...
	logger.Info("Compiling with tsgo...")
	err := o.Compiler.Run(ctx, compiler.RunOptions{
		TsConfigPath: o.NestConfig.CompilerOptions.TsConfigPath, // Use config value or detected one
	})
	if err != nil {
		return fmt.Errorf("compilation failed: %w", err)
	}

	if len(o.TsConfig.Paths) > 0 {
		logger.Info("Rewriting path aliases in %s...", o.TsConfig.OutDir)
		if err := o.Rewriter.RewriteDir(o.TsConfig.OutDir); err != nil {
			return fmt.Errorf("path alias rewrite failed: %w", err)
		}
	}
...
func (o *Orchestrator) Watch(ctx context.Context, watchAssets bool) error {
	...
	wt, err := watcher.New(ctx, absSourceRoot, 300*time.Millisecond, func() {
		select {
